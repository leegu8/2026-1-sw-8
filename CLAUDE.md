...
```
2026-1-sw-8/
├── requirements.txt          # backend 전용 (gaze 의존성 없음)
├── frontend/
...
│           ├── services/gazeSocket.js # ws://localhost:8765/ws → CustomEvent 변환
│           └── api/gazeApi.js        # REST API 호출 (localhost:8765 기준)
├── backend/app/
│   ├── main.py               # FastAPI 앱 진입점, 라우터 등록, seed 데이터
│   ├── core/config.py        # DATABASE_URL만 포함
...
│   ├── __main__.py           # python -m gaze_client 진입점
│   ├── gaze_server.py        # FastAPI, CORS allow_origins=["*"]
│   ├── config.py             # 시선추적 상수 (DEADZONE_PX, Y_CORRECTION_K 등)
│   ├── model_loader.py       # MediaPipe 모델 다운로드·로드
│   ├── requirements.txt      # mediapipe, opencv, numpy, fastapi, uvicorn
│   └── gaze/
│       ├── tracker.py        # 웹캠 캡처 루프, 칼만 필터
│       ├── calibration.py    # Ridge Regression 보정 모델
│       ├── feature_extractor.py # 눈 패치 120차원 특징 추출
│       └── visualizer.py    # 얼굴 랜드마크 시각화
└── deploy/                   # Render 배포용 (backend + frontend만)
    ├── backend/
    ├── frontend/
    └── requirements.txt
```
...
- **reading** (사용자): 역행 블러만 적용. 페이지네이션·완독률 계산·역행비율 pending 버퍼 방식. 마지막 페이지에서만 완료 버튼 활성
- **reading-admin** (관리자): 실시간 하이라이트·역행 블러 개입. DEV_MODE(마우스=시선). 분석 지표 프론트에서 계산 후 DB 저장. BLUR/HIGHLIGHT 교정 이벤트는 종료 버튼 클릭 시 한 번에 DB 저장. 완독률: 방문 세그먼트 Set 합산 / 전체(줄×5). 역행비율: left pending 버퍼 → right 발생 시 줄바꿈 여부 판단
...
```
gaze_client (웹캠 → MediaPipe → Ridge Regression, 포트 8765)
  → WebSocket ws://localhost:8765/ws (30fps JSON)
...
## 주요 상수 (gaze_client/config.py)

| 상수 | 값 | 설명 |
|------|----|------|
| `SMOOTH_ALPHA` | 0.10 | EMA 시선 스무딩 계수 |
| `DEADZONE_PX` | 6 | 시선 이동 감지 최소 픽셀 |
| `Y_CORRECTION_K` | 0.00004 | Y축 하향 편향 보정 계수 |
| `Y_GAIN` | 1.5 | Y축 감도 배율 |
...
### 시선 추적 (gaze_client, 포트 8765)

| 메서드 | 경로 | 설명 |
|--------|------|------|
| `GET` | `/api/status` | 시스템 상태 |
| `POST/DELETE` | `/api/calibrate` | 보정 포인트 추가 `{x, y, count}` / 초기화 |
| `POST` | `/api/calibrate/y-correction?active=true\|false` | Y좌표 보정 활성화 토글 |
...
| `GET` | `/api/db/sessions/{id}/result` | 세션 결과 |
| `POST` | `/api/db/correction-events` | 교정 이벤트 저장 — 종료 시 일괄 전송 |
| `GET` | `/api/db/users/{id}/growth` | 성장일지 — 최근 5세션 요약·점수 |

## 코딩 규칙

- **아키텍처**: SRP — `GazeTracker`는 캡처 루프만, 특징 추출·보정·시각화는 전담 클래스에 위임
- **상태 공유**: `app.state.tracker` gaze_client FastAPI 앱 전역 싱글턴
...
- 시선추적은 `gaze_client`가 실행 중이어야 동작
- 보정은 **4개 포인트** 이상 수집 시 활성화 (`calibration.py: _MIN_POINTS=4`)
- `face_landmarker.task`는 `.gitignore`에 포함 (자동 다운로드)
- Render 배포 시 Mixed Content 문제: Chrome 플래그에서 `http://localhost:8765` 허용 필요

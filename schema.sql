...
-- ------------------------------------------------
-- 9. TEXT_WORD (단어 + 툴팁 뜻 저장)
-- ------------------------------------------------
CREATE TABLE TEXT_WORD (
  id             BIGINT       NOT NULL AUTO_INCREMENT,
  text_id        BIGINT       NOT NULL,
  word           VARCHAR(100) NOT NULL COMMENT '단어 원형',
  meaning        TEXT         DEFAULT NULL COMMENT '툴팁에 표시할 뜻',
  sentence_index INT          NOT NULL COMMENT '몇 번째 문장인지',
  word_index     INT          NOT NULL COMMENT '문장 내 몇 번째 단어인지',
  PRIMARY KEY (id),
  CONSTRAINT fk_word_text FOREIGN KEY (text_id) REFERENCES TEXT_CONTENT(id) ON DELETE CASCADE,
  INDEX idx_word_text (text_id, sentence_index)
) ENGINE=InnoDB;

-- ------------------------------------------------
-- 10. WORD_GAZE_LOG (단어별 응시 기록)
-- ------------------------------------------------
CREATE TABLE WORD_GAZE_LOG (
  id            BIGINT    NOT NULL AUTO_INCREMENT,
  session_id    BIGINT    NOT NULL,
  word_id       BIGINT    NOT NULL,
  fixation_ms   INT       NOT NULL COMMENT '총 응시 시간 (ms)',
  tooltip_shown TINYINT(1) NOT NULL DEFAULT 0 COMMENT '툴팁 표시 여부 (1.5초 초과시 1)',
  recorded_at   DATETIME  NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  CONSTRAINT fk_wordgaze_session FOREIGN KEY (session_id) REFERENCES READING_SESSION(id) ON DELETE CASCADE,
  CONSTRAINT fk_wordgaze_word    FOREIGN KEY (word_id)    REFERENCES TEXT_WORD(id)        ON DELETE CASCADE
) ENGINE=InnoDB;